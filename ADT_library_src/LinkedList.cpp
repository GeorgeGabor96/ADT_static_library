...
int PutLinkedListPush(LinkedList list, void * element)
{
	return PutLinkedListItem(list, element, 0);
}

void * DeleteLinkedListPop(LinkedList list)
{
	if (list->nr_elemente == 0)
	{
		return NULL;
	}
	nod * nodSters = list->first;
	if (list->nr_elemente == 1)
	{
		list->first = NULL;
		list->last = NULL;
	}
